...
#ifndef SINGULARITY_OPAC_NEUTRINOS_NON_CGS_NEUTRINOS_
...
class NonCGSUnits {
 public:
  ...
  EmissivityPerNuOmega(const Real rho, const Real temp, const Real Ye,
    ...
    opac_.EmissivityPerNuOmega(rho * rho_unit_, temp * temp_unit_, Ye, type,
                               nu_bins, coeffs, nbins, lambda);
    for (int i = 0; i < nbins; ++i) {
      nu_bins[i] *= time_unit_;
  ...
  Real EmissivityPerNu(const Real rho, const Real temp, const Real Ye,
                       const RadiationType type, const Real nu,
                       Real *lambda = nullptr) const {
    Real Jnu = opac_.EmissivityPerNu(rho * rho_unit_, temp * temp_unit_, Ye,
                                     type, nu * freq_unit_, lambda);
  ...
  EmissivityPerNu(const Real rho, const Real temp, const Real Ye,
    ...
    opac_.EmissivityPerNu(rho * rho_unit_, temp * temp_unit_, Ye, type, nu_bins,
                          coeffs, nbins, lambda);
    for (int i = 0; i < nbins; ++i) {
      nu_bins[i] *= time_unit_;
      coeffs[i] *= inv_emiss_unit_;
  ...
  Real NumberEmissivity(const Real rho, const Real temp, Real Ye,
    ...
    Real JoH = opac_.NumberEmissivity(rho * rho_unit_, temp * temp_unit_, Ye,
                                      type, lambda);
  ...
  Real ThermalDistributionOfTNu(const Real temp, const RadiationType type,
    ...
    Real BoH = opac_.ThermalDistributionOfTNu(temp * temp_unit_, type,
  ...
  Real ThermalNumberDistributionOfT(const Real temp, const RadiationType type,
                                    Real *lambda = nullptr) const {
    Real NoH =
        opac_.ThermalNumberDistributionOfT(temp * temp_unit_, type, lambda);
